angular.module('pegasus').controller("RegionsController",
	[
	 	"$rootScope", "$scope", "$location", "$state", "SecurityService", "ErrorService", "RegionService",
	 	function ($rootScope, $scope, $location, $state, SecurityService, ErrorService, RegionService) {
	 		$scope.busy = false;
	 		...
	 		// columns
	 		$scope.columnHeaders = [
	            { label: "Region", value: "name", sortable: true },
	 		...
	 		$scope.find = function() {
	 			...
	 			// store in root scope
	 			$rootScope.regionsSearchSM = {
	 				opinion: $scope.sm.opinion,
	 				searchFilter: $scope.sm.searchFilter
	 			};

	 			RegionService.find($scope.sm.searchFilter).then(function(response) {
	 		...
	 		$scope.changeOpinion = function(opinion) {
	 			...
	 			// store in root scope
	 			$rootScope.regionsSearchSM.opinion = opinion;
	 		};
	 		 ...
	 		 */
	 		$scope.add = function() {
		 		$state.go("region", { "regionId" : "new" });
		 	};
		 	
		 	$scope.loadFilters = function() {
		 		$scope.filtersBusy = true;

		 		if ($rootScope.regionsSearchSM) {
		 			applyStateMachine();
		 		} else {
		 	...
		 	function applyStateMachine() {
				$scope.pagination.pageIndex = $rootScope.regionsSearchSM.searchFilter.pageIndex;
				$scope.pagination.itemsPerPage = $rootScope.regionsSearchSM.searchFilter.itemsPerPage;
				$scope.pagination.sort.property = $rootScope.regionsSearchSM.searchFilter.sortField;
				$scope.pagination.sort.direction = $rootScope.regionsSearchSM.searchFilter.sortDirection;
				$scope.sm.opinion = $rootScope.regionsSearchSM.opinion;
		 	}
