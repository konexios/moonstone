...
// TODO revisit the below directives once ajaxtables have been implemented
...
directives.directive("arrowItemsPerPageDropdown", function() {
	return {
		...
		templateUrl: "/scripts/pegasus/tpl/ItemsPerPageDropdown.html"
	}
});
...
directives.directive("arrowPagination", function() {
	return {
		...
		templateUrl: "/scripts/pegasus/tpl/Pagination.html"
	}
});
...
directives.directive("arrowShowingItems", function() {
	return {
		...
		templateUrl: "/scripts/pegasus/tpl/ShowingItems.html"
	}
});
...
directives.directive("arrowColumnHeader", function() {
	return {
		...
		templateUrl: "/scripts/pegasus/tpl/ColumnHeader.html"
	}
});
...
directives.directive("leftMenu", function() {
	...
	var directive = {
		...
		templateUrl: 'scripts/pegasus/tpl/left-menu.tmp.html',
...
directives.directive('emailValidation', function() {
    ...
    return {
        ...
        link: function(scope, element, attributes, model) {
            ...
            if (model && model.$validators.email) {
                model.$validators.email = function(modelValue) {
                    return model.$isEmpty(modelValue) || EMAIL_REGEXP.test(modelValue);
                };
            }
        }
    }
...
directives.directive('loginValidation', function() {
    var LOGIN_REGEXP = /([^A-Za-z0-9@]).*/i;
    var EMAIL_REGEXP = /^(([^<>()\[\]\.,;:\s@\"]+(\.[^<>()\[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i;
    ...
    return {
        ...
        link: function(scope, element, attributes, model) {
            if (model) {
                model.$validators.login = function(modelValue) {

                	if (!modelValue) return false;

                	if (modelValue.indexOf("@") === -1) {
                        return model.$isEmpty(modelValue) || !LOGIN_REGEXP.test(modelValue);
                    } else {
                        return model.$isEmpty(modelValue) || EMAIL_REGEXP.test(modelValue);
					}
                };
...
directives.directive('phoneFormatter', function () {
    return {
        restrict: 'A',
        require: 'ngModel',
        link: function (scope, element, attributes, model) {
            scope.$watch(attributes.ngModel, function (v) {
				...
				model.$viewValue = v.replace(/[^0-9]/gim,'')
                     				.replace(/(\d)(\d\d\d)(\d\d\d)(\d\d\d\d)/, '+$1($2)-$3-$4');
                model.$render();
            });
        }
    };
...
directives.directive('postcodeFormatter', function () {
    return {
        restrict: 'A',
        require: 'ngModel',
        link: function (scope, element, attributes, model) {
