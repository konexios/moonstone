...
@Widget(directive = "event-last-registered-widget", name = "Social Event Last Registration Widget", description = "A widget that shows the statistics of social event registrations.")
public class SocialEventLastRegisteredWidget extends ScheduledWidgetAbstract {

	@DataProvider
	private SocialEventDataProvider socialEventDataProvider;
	@TopicProvider("/last-registered")
	private SimpleTopicProvider lastRegisteredTopicProvider;
	...
	public Configuration processDeviceSelection(Configuration configuration) {
		...
		populateSocialEvent(configuration);
	...
	public SocialEventLastRegisteredWidget() {
		super(1);
	...
	public void run() {
		...
		addScheduledWorker(new ScheduledWorker(), 0, 5, TimeUnit.SECONDS);
	}
	...
	private void populateSocialEvent(Configuration configuration) {
		String method = "populateSocialEvent";
		logInfo(method, "...");
	...
	private class ScheduledWorker implements Runnable {
		...
		public void run() {
			...
			try {
				List<SocialEventLastRegistered> lastRegisteredBulk = new ArrayList<>();
				Instant newLastRequestTime = Instant.now();
				if (timeZoneId != null) {
					...
					lastRegisteredBulk = socialEventDataProvider.requestLastRegisteredBulk(socialEvent.getId(),
					        lastRequestTime, zoneOffset);
					lastRequestTime = newLastRequestTime;

					if (lastRegisteredBulk.size() < 4) {
						int diff = 4 - lastRegisteredBulk.size();
						logInfo(method, "diff: %s", diff);

						List<SocialEventLastRegistered> lastRegisteredList = socialEventDataProvider
						        .requestLastRegistered(socialEvent.getId(), diff);

						if (!lastRegisteredList.isEmpty()) {
							lastRegisteredBulk.addAll(lastRegisteredList);
							lastRegisteredBulk.sort(Comparator.comparing(SocialEventLastRegistered::getTimestamp));
						}
					}
				}
				logDebug(method,
				        "last registered bulk for social event" + socialEvent.getId() + ": " + lastRegisteredBulk);
				sendMessage(lastRegisteredTopicProvider,
				        new WidgetData().withState(getState()).withData(lastRegisteredBulk));
			} catch (Throwable t) {
				logError(method, "error refreshing social event last registered", t);
			}
		}
	}
}
